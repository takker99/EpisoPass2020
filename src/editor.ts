...
export const editor = (data: EpisoData) => {
  ...
  const answerspan = function (q: number, a: number) { // q番目の質問のa番目の選択肢のspan
  ...
  const showimage = function (str: string, img: JQuery<any>) {
  ...
  const qadiv = function (q: number) { // q番目の質問+選択肢のdiv
    answer[q] = 0;
  ...
  const maindiv = function () {
    ...
    const minus = $(
      ...
      .click(function () { // 質問の数を減らす「-」ボタンをクリックしたとき呼ばれる関数
        qas.pop();
    ...
    const plus = $('<input type="button" value=" 問題追加 " class="qabutton">')
      .click(function () { // 質問の数を増やす「-」ボタンをクリックしたとき呼ばれる関数
        qas.push({
          question: "新しい質問",
